...
5. Follow ChainOfThought progress as the approved lookup emits Sources and streams a Markdown Artifact. Rejection skips report generation.
6. Copy or download the report after the AI SDK reports successful completion. Closing the panel does not discard the report; reopen it to continue reviewing.
7. Ask another question once approval and generation have finished.
...
The demo sources use example.com placeholder URLs and fixed note titles. Progress comes from actual tool states and stream completion, not a separate timer. Copy and download stay disabled for partial or interrupted reports. Approval-request completion alone does not mark the report complete. The application owns clipboard handling, download Blob URLs, panel visibility, and cleanup when the chat unmounts.
