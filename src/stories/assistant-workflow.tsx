...
function Workflow() {
  const [transport] = useState(() =>
    createApprovalTransport({ reasoning: true, report: true }),
  );
  ...
  const chat = useChat({
    onFinish: ({ message, isAbort, isError, isDisconnect }) => {
      if (
        ...
        setCompleted((previous) => new Set(previous).add(message.id));
    },
    transport,
  ...
  return (
    <Stack>
      <Text size="sm" c="dimmed">
        ...
        reasoning, tools, sources, progress, and an exportable report. All data
        and source links are fixed demo fixtures.
      </Text>
      {!questionDone && (
        <Question
            ...
            <Message key={message.id} from={message.role}>
              <Message.Content>
                {message.role === 'assistant' && (
                  <WorkflowReport
                    message={message}
                    complete={completed.has(message.id)}
                    busy={busy && message.id === chat.messages.at(-1)?.id}
                  />
                )}
...
export function AssistantWorkflowExample() {
  return (
    <PromptInputProvider>
      <Workflow />
    </PromptInputProvider>
