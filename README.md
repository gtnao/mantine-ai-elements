...
| `Sandbox` | Tool execution status with code and output tabs |
| `JSXPreview` | Trusted JSX previews with Mantine components and streaming recovery |
| `WebPreview` | Iframe previews with URL navigation and application-supplied console logs |
| `Snippet` | Read-only commands with optional prefixes and clipboard actions |
| `Artifact` | Generated document panels with application-owned actions and scrolling content |
| `ChainOfThought` | Application progress steps, source badges, and image previews |
| `Plan` | Collapsible execution plans with streaming titles, descriptions, and application actions |
